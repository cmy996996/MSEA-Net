...
class ConfusionMatrix(object):
    ...
    def compute(self):
        ...
        # 添加精确率(Precision)计算
        # 真正例（TP）除以预测为正的所有样本（TP + FP）
        precision = torch.diag(h) / h.sum(0)

        # 添加召回率(Recall)计算
        # 真正例（TP）除以所有实际为正的样本（TP + FN）
        # 注意：由于召回率需要考虑真实正样本的总数，这里我们假设混淆矩阵的行对应真实类别，列对应预测类别
        # 因此，每一列的真实正样本数需要通过对角线元素求和获得（因为对角线外的元素不计入真实正样本）
        true_positives_per_class = torch.diag(h)
        actual_positives_per_class = h.sum(1)
        recall = true_positives_per_class / actual_positives_per_class

        # 确保除法中没有除以零的情况，可选：用eps（极小值）来避免
        eps = 1e-6
        precision = torch.clamp(precision, min=eps)
        recall = torch.clamp(recall, min=eps)
        f1_score = 2 * (precision * recall) / (precision + recall)

        # 确保 F1 Score 中没有 NaN 或 Inf
        f1_score = torch.nan_to_num(f1_score, nan=0.0, posinf=0.0, neginf=0.0)
    ...
    def __str__(self):
        ...
        # 将精度和召回率转换为百分比并格式化
        iu_percent = ['{:.2f}'.format(i * 100) for i in iu.tolist()]
        iu_str = '*'.join(iu_percent)  # 使用星号连接IoU值

        precision_percent = ['{:.2f}'.format(p * 100) for p in precision.tolist()]
        recall_percent = ['{:.2f}'.format(r * 100) for r in recall.tolist()]
        f1_score_percent = ['{:.2f}'.format(f * 100) for f in f1_score.tolist()]
        ...
        return (
            ...
            'Precision per class: {}\n'
            'Recall per class: {}\n'
            'F1 Score per class: {}').format(
            acc_global.item() * 100,
            ['{:.2f}'.format(i) for i in (acc * 100).tolist()],
            iu_str,  # 使用带有星号的IoU字符串
            iu.mean().item() * 100,
            precision_percent,
            recall_percent,
            f1_score_percent)
...
class DiceCoefficient(object):
    ...
    def update(self, pred, target):
        ...
        print(multiclass_dice_coeff(pred[:, 1:], dice_target[:, 1:], ignore_index=self.ignore_index))
        self.count += 1
